...
if [ -f /tmp/fresh-cluster ]; then
  ...
  sed -i -e "s/CLOUDFLARE_API_KEY/$(echo -n ${cloudflare-api-key} | base64)/" -e "s/CLOUDFLARE_API_EMAIL/$(echo -n ${cloudflare-api-email} | base64)/" /tmp/manifests/external-dns.yaml
